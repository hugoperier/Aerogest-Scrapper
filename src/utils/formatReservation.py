...
def email_format(reservations):
    today = datetime.now()
    message = f"        <h2>Fetched on: {today.strftime('%A %d %B %Y %H:%M')}</h2>\n"
    for reservation in reservations:
        ...
        message += f"            <p>{reservation['schedule']['start'].strftime('%d/%m/%Y')} {reservation['schedule']['start'].strftime('%H:%M')} - {reservation['schedule']['end'].strftime('%H:%M')}</p>\n"
        message += f"        </div>\n"
    return message
